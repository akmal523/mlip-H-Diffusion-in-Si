...
# --- Structural variant -------------------------------------------------------
# "surface"       — H on top Si surface; vacuum gap in z
#                   relevant for surface/near-surface diffusion
# "interstitial"  — H at tetrahedral T-site inside Si bulk; fully periodic box
#                   relevant for bulk diffusion (professor's recommended setup)
# "both"          — run both variants; results in separate subdirectories
#
# Run  python create_structures.py  first to generate the structure files.
STRUCT_VARIANT = "interstitial"   # "surface" | "interstitial" | "both"

# Structure files (generated by create_structures.py)
_STRUCT_FILES = {
    "surface":      "si_with_h_surface.lmp",
    "interstitial": "si_with_h_interstitial.lmp",
}
_STRUCT_NAMES = {
    "surface":      "Si64H1_surface",
    "interstitial": "Si64H1_interstitial",
}
...
def _run_variant(struct_file: str, struct_name: str) -> None:
    """Launch all temperature jobs for one structural variant."""

    if not os.path.isfile(struct_file):
        raise FileNotFoundError(
            f"Structure file not found: {struct_file}\n"
            "Run  python create_structures.py  first."
        )
    ...
    print(
        f"\n{'='*55}\n"
        f"  Variant: {struct_name}\n"
        f"{'='*55}\n"
        f"  Structure : {struct_file}\n"
        f"  Slab area      = {area:.4f} Å²\n"
        f"  Slab thickness = {slab_thick:.4f} Å\n"
        f"  Slab volume    = {slab_vol:.4e} Å³\n"
    )
    ...
    print(
        f"\n  {len(TEMPERATURES)} jobs launched."
        f"  Monitor: ./monitor_jobs.sh --watch"
    )


def main() -> None:
    if STRUCT_VARIANT not in ("surface", "interstitial", "both"):
        raise ValueError(
            f"Unknown STRUCT_VARIANT: '{STRUCT_VARIANT}'.\n"
            "Set it to 'surface', 'interstitial', or 'both'."
        )

    print(f"[config] STRUCT_VARIANT = {STRUCT_VARIANT}")
    print(f"[config] MODE           = {MODE}")

    variants = (
        list(_STRUCT_FILES.items())
        if STRUCT_VARIANT == "both"
        else [(STRUCT_VARIANT, _STRUCT_FILES[STRUCT_VARIANT])]
    )

    for variant_key, struct_file in variants:
        _run_variant(struct_file, _STRUCT_NAMES[variant_key])

    print("\n" + "="*55)
    print("All jobs launched.")
    print("="*55)
    print("\nMonitor:  ./monitor_jobs.sh --watch")
    print("\nAnalyze after completion:")
    for variant_key, _ in variants:
        name = _STRUCT_NAMES[variant_key]
        print(f"  python analyze_msd.py --run-folder {RUN_FOLDER}/{name}  # {variant_key}"
    )
